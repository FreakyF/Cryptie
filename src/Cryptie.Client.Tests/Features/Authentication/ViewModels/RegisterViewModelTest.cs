...
//             var _ = viewModel.RegisterCommand.CanExecute.FirstAsync().Wait();
//             // ErrorMessage powinien być ustawiony przez walidację
...
//             viewModel.Model.Email = "test@example.com";
//             await viewModel.RegisterCommand.Execute();
//             Assert.Equal("MOCK_PRIVATE_KEY_ABC123", capturedDto?.PrivateKey);
//             Assert.Equal("MOCK_PUBLIC_KEY_DEF456", capturedDto?.PublicKey);
//         }
//
//         [Fact]
//         public async Task RegisterCommand_Should_ShowRegister_On_Cancellation()
//         {
//             var viewModel = CreateViewModel();
//             var mockResponse = new RegisterResponseDto { Secret = "secret", TotpToken = Guid.NewGuid() };
//             _authServiceMock.Setup(a => a.RegisterAsync(It.IsAny<RegisterRequestDto>(), It.IsAny<CancellationToken>()))
...
//                 await viewModel.RegisterCommand.Execute(cts.Token));
//             // ShowRegister powinno być wywołane, ale tylko jeśli kod dotrze do tego miejsca (w tym teście rzuca wyjątek)
//         }
//     }
// }
